...
def get_time_stamp(checkpoint_dir: str = None) -> str:
    if checkpoint_dir:
        checkpoint_dir = osp.realpath(checkpoint_dir)
        assert osp.exists(checkpoint_dir), checkpoint_dir
        for _s in str(osp.basename(checkpoint_dir)).split('-'):
            if len(_s) == 11 and _s[4] == '_':
                time_stamp = _s
                break
    else:
...
def auto_make_exp_dir(main_file: str, time_stamp: str = "", data_name: str = "", exp_root='save.exps', mkdir: bool = True) -> str:
    proj_root, sub_dir = osp.split(osp.dirname(osp.realpath(main_file)))
    file_name = osp.splitext(osp.basename(main_file))[0]
    exp_dir = osp.join(proj_root, exp_root, sub_dir, f"{file_name}{'-' + data_name if data_name else ''}{'-' + time_stamp if time_stamp else ''}")
    if mkdir:
        if not osp.exists(exp_dir):
            os.makedirs(exp_dir)
        else:
            warnings.warn(f"the exp_dir {exp_dir} has been existed! It will be overwriten!")
    return exp_dir


def get_logger(log_dir: str, file_name: str = 'train_eval.log', log_name: str = None) -> logging.Logger:
...
def backup_codes(main_file: str, dst_dir: str, overwrite: bool) -> list[str]:
    if not osp.exists(dst_dir):
        os.makedirs(dst_dir)
    src_dir = osp.dirname(osp.realpath(main_file))
    assert osp.exists(src_dir)
    src_file_list = []
    for dirpath, dirnames, filenames in os.walk(src_dir):
        if not '__pycache__' in dirpath.split(osp.sep):
            for filename in filenames:
                src_file_list.append(osp.join(dirpath, filename))

    copied_files = []
    for src_file in src_file_list:
        if osp.isfile(src_file):
            src_rel_path = osp.relpath(src_file, src_dir)
            dst_file_dir = osp.join(dst_dir, osp.dirname(src_rel_path))
            if not osp.exists(dst_file_dir):
                os.makedirs(dst_file_dir)
            dst_file_path = osp.join(dst_file_dir, osp.basename(src_file))
            if not osp.exists(dst_file_path) or overwrite:
                shutil.copy(src_file, dst_file_path)
                copied_files.append(dst_file_path)
    return copied_files


def load_params(source: str | dict, model: torch.nn.Module, *param_names: str, strict: bool = False, remove_prefix="_orig_mod.", ignore_patterns: tuple[str] = (), verbose: bool = False) -> list[str]:
    param_dict = {'base_params': None, 'task_params': None}
    assert len(param_names) <= len(param_dict), "Only support base- and task-params now."
    ...
    for n in list(mearged_state_dict.keys()):
        if ignore_patterns:
            if any([(s in n) for s in ignore_patterns]):
                mearged_state_dict.pop(n)

        if remove_prefix and n in mearged_state_dict.keys():
    ...
    loaded_params = list(mearged_state_dict.keys())
    if verbose:
