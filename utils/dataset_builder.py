...
class EuroSATPath(ImagePathDataset):
    def __init__(self, root_dir: str, train: bool):
        super().__init__()
        cls_dir_list = sorted(os.listdir(split_path := join(root_dir, 'train_2024' if train else 'val_2024')))
        assert exists(split_path), split_path
        self.split_path = split_path
        ...
        assert len(self.class_imgpath_dict) == 10


class RESISCPath(ImagePathDataset):
    def __init__(self, root_dir: str, train: bool):
        super().__init__()
        cls_dir_list = sorted(os.listdir(split_path := join(root_dir, 'train_2024' if train else 'val_2024')))
        assert exists(split_path), split_path
        ...
        assert len(self.class_imgpath_dict) == 45


class OmniBenchmarkPath(ImagePathDataset):
    def __init__(self, root_dir: str, train: bool):
        super().__init__()
        cls_dir_list = sorted(os.listdir(split_path := join(root_dir, 'train' if train else 'val')))
        assert exists(split_path), split_path
        ...
        assert len(self.class_imgpath_dict) == 300, f"{len(self.class_imgpath_dict)}"
...
class ImagePathDatasetClassManager():
    def __init__(self, **kwargs):
        self.dataset_dict = {
            ...
            'eurosat': partial(EuroSATPath, root_dir="../datasets/data.EuroSAT" if not (v := kwargs.get('eurosat')) else v),
            'resisc': partial(RESISCPath, root_dir="../datasets/data.RESISC" if not (v := kwargs.get('resisc')) else v),
            'omni': partial(OmniBenchmarkPath, root_dir="../datasets/data.OmniBenchmark" if not (v := kwargs.get('omni')) else v),
        }
