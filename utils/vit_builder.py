...
class Prompt(nn.Module):
    def __init__(self, prompt_len: int, embed_dim: int = 768, prompt_init: str = 'uniform', replace_token: bool = False, prompt_after_norm: bool = False, pre_tokens: int = 197):
        super().__init__()
        assert prompt_len >= 0
        ...
        self.__pre_tokens = pre_tokens
        self.__prompt_after_norm = prompt_after_norm
        ...
        if prompt_init == 'zero':
            ...
            prompt_init_value = 1
            nn.init.uniform_(self.prompt, -prompt_init_value, prompt_init_value)
        elif prompt_init in ('trunc_normal',):
            self.prompt = nn.Parameter(torch.randn(1, prompt_len, embed_dim))
            nn.init.trunc_normal_(self.prompt)
        elif prompt_init in ('xavier_uniform',):
            self.prompt = nn.Parameter(torch.randn(1, prompt_len, embed_dim))
            prompt_init_value = 1
            nn.init.xavier_uniform_(self.prompt, prompt_init_value)
        elif prompt_init in ('kaiming_uniform',):
            self.prompt = nn.Parameter(torch.randn(1, prompt_len, embed_dim))
            prompt_init_value = 1
            nn.init.kaiming_uniform_(self.prompt, prompt_init_value)
        else:
    ...
    @property
    def prompt_after_norm(self):
        return self.__prompt_after_norm
    ...
    def extra_repr(self) -> str:
        return f"prompt_len={self.prompt_len}, embed_dim={self.embed_dim}, prompt_init={self.prompt_init}, replace_token={self.replace_token}, prompt_after_norm={self.prompt_after_norm}"
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if hasattr(self, 'prompt'):
            ...
            if self.prompt.prompt_after_norm:
                raise NotImplementedError()
                _fn1, _fn2 = self.norm1, self.prompt
            else:
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        print('+'*60, f"embed_len = {embed_len}")
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                act_layer=act_layer,
                mlp_layer=mlp_layer,
                prompt=Prompt(_pd['prompt_len'], embed_dim, _pd['prompt_init'], False, False, embed_len) if _pd['prompt_start_block'] <= i <= _pd['prompt_end_block'] and _pd['prompt_len'] > 0 else None
            )
